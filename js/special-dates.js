(function registerSpecialDatesRepository(window) {
  ...
  if (!dates) {
    throw new Error(
      "special-dates.js requer date-utils.js carregado previamente.",
    );
  }
  ...
  function normalizeTextList(value, fieldName) {
    ...
    if (normalizedValue.length === 0) {
      throw new Error(
        `O campo ${fieldName} precisa conter ao menos um texto válido.`,
      );
    }
  ...
  function createSpecialDate(type, name, summary, motivation) {
    ...
    if (safeSummary.length === 0) {
      throw new Error(
        `A data especial "${safeName}" precisa ter ao menos um resumo.`,
      );
    }

    if (safeMotivation.length === 0) {
      throw new Error(
        `A data especial "${safeName}" precisa ter ao menos uma mensagem.`,
      );
    }
  ...
  function addSpecialDate(repository, date, specialDate) {
    if (!repository || typeof repository !== "object") {
      throw new TypeError(
        "O repositório de datas especiais precisa ser um objeto.",
      );
    }
  ...
  function getSpecialDates(year) {
    ...
    if (safeYear < 1583) {
      throw new RangeError(
        "Ano inválido informado para consulta de datas especiais.",
      );
    }
